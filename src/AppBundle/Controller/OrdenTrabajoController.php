...
class OrdenTrabajoController extends Controller {
    ...
    public function xxremoveEquipoAsociadoAction(Request $request) {
        //UtilsController::haveAccess($this->getUser(),'ordentrabajo_new');
        $em = $this->getDoctrine()->getManager();
        $form = $request->get('appbundle_estadoubicacion');
        if ($form) {
            ...
            $otDet = $em->getRepository('AppBundle:OrdenTrabajoDetalle')->find($request->get('id'));
            $src = $request->get('src');
        }
        if (!$otDet) {
            throw $this->createNotFoundException('No se encuentra.');
        ...
        try {
            $em->getConnection()->beginTransaction();
            $otId = $otDet->getOrdenTrabajo()->getId();
            $reqdet = $otDet->getRequerimientoDetalle();
            $equipo = $otDet->getEquipo();
            ...
            if ($form) {
                // No hay ubicacion original - se carga del modal
                $concepto_entrega = $em->getRepository('ConfigBundle:ConceptoEntrega')->find($form['conceptoEntrega']);
                $estado = $em->getRepository('ConfigBundle:Estado')->find($form['estado']);
                $equipo->setEstado($estado);
                $departamento = $em->getRepository('ConfigBundle:Departamento')->find($form['departamento']);
                $piso = $em->getRepository('ConfigBundle:Piso')->find($form['piso']);
            }
            else {
                // generar ubicacion segun original
                $equipo->setEstado($otDet->getEstadoOriginal());
                $ubicOriginal = $otDet->getEquipoUbicacionOriginal();
                $concepto_entrega = $ubicOriginal->getConceptoEntrega();
                $departamento = $ubicOriginal->getDepartamento();
            ...
            $reqId = $otDet->getOrdenTrabajo()->getRequerimiento()->getId();
            // verificar a donde volver
            if ($src == 'OT') {
                // volver a edicion de requerimiento solapa de la OT
                $session = $this->get('session');
                $session->set('otid', $otId);
                $route = $this->generateUrl('soporte_requerimiento_edit', array('id' => $reqId));
            }
            else {
                // volver a tareas
                $route = $this->generateUrl('soporte_ordentrabajo_tareas', array('id' => $otId));
            }
